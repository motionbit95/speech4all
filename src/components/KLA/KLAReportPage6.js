...
const KLAReportPage6 = forwardRef(
  ({ customData, setCurrentPage, subPage }, ref) => {
    ...
    return (
      <Box ref={ref}>
        {pages.map((page, pageIndex) => {
          ...
          console.log(">>>>>>>>>>>>:", pageIndex, subPage);
          if (pages.length - pageIndex === subPage) {
            return null;
          } else {
            return (
              <Box
                onClick={() => setCurrentPage(pageIndex + 1)} // You can pass pageIndex + 1 to indicate page number
                key={pageIndex}
                ref={(el) => {
                  if (!ref.current) {
                    ref.current = []; // 배열로 초기화
